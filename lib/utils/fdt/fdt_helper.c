...
static int fdt_parse_isa_extensions_one_hart(const char *isa, unsigned long *extensions, int len)
{
	size_t i;
...
static int fdt_parse_isa_all_harts(void *fdt)
{
	...
	fdt_for_each_subnode(cpu_offset, fdt, cpus_offset) {
		...
		if (val && len > 0) {
			err = fdt_parse_isa_extensions_one_hart((const char *)val,
								hart_exts, len);
			if (!err)
				continue;
		}
